...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    if (storedPrice) {
      setPrice(storedPrice); // Set price in state
    } else {
      console.error("Error: Order price missing in session storage");
    }
  }, []);

  const userUpiId = "shruti.awate1502@okicici"; // Replace with your personal UPI ID
const userName = "SHRUTI AWATE"; // Your name as registered on UPI

const paymentLinks = {
  upi: `upi://pay?pa=${userUpiId}&pn=${userName}&tr=${orderId}&am=${price}&cu=INR`,
